...
class CartController {
	getMyCartId = (userId: string): string | undefined =>
		userId.split(':')[1] === undefined
			? undefined
			: `${cartService.tablename + ':' + userId.split(':')[1]}`;
	...
	async getUserCart(userId: string): Promise<AppError | CartData> {
		...
		return (
			(await cartService.findOneById<CartData>(
				cartId,
				['items', 'items.menuItem', 'items.menuItem.menuItemTypes'],
				cartDataSchema
			)) ?? new InternalServerError('error fetching cart')
		);
	}
	...
	async updateCart(userId: string, data: CartUpdateInput): Promise<AppError | CartModel> {
		...
		if (data.add) {
			...
			log.warn(cart.items);
			const index = cart.items.findIndex((e) => e.menuItem === data.delete);
